...
var MyUtils=
{
	...
	browser:function()
	{
		...
		_browser:null,
	className:function(elements,str)
	{
		if(!MyUtils.isList(elements))
			elements=[elements];
		for(var i=0, iLen=elements.length; i<iLen; i++)
	...
	{
		if(!MyUtils._requestAnimationFrame)
			MyUtils._requestAnimationFrame=window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame || window.msRequestAnimationFrame;
		MyUtils._requestAnimationFrame.call(window, callback);
	},
